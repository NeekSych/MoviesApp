...
class Cards extends Component{
    render(){
    const { poster_path, release_date, title, overview, genre_ids } = this.props.movieProps
...
>
    ...
    className = 'card__flex'>
      ...
      >
        ...
        {genre_ids.map((genre)=>{
            return (
            <Text code id={genre.id}>{genre.name}</Text>
                )
        }
        )}
        </div>
        <Text className='card__description'>{cardDescription(overview)}</Text>
      </Flex>
    </Flex>
  </Card>
)}};
